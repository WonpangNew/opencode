...
A general-purpose agent for researching complex questions and executing multi-step tasks. Has full tool access (except task list), so it can make file changes when needed. Use this to run multiple units of work in parallel.
