...
Manage subagent orchestration plans during coding sessions.
...
Creates and updates plans for launching subagents via the Task tool. Each plan item specifies subagent_type and prompt for the Task tool to execute. The LLM uses this to organize multi-step subagent work.
...
Read existing subagent orchestration plans.
...
Reads the current subagent plan state. Used by the LLM to track which subagent tasks are pending or completed.
